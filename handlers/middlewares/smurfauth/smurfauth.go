...
func (a *Middleware) AuthHandler(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		smurf, err := a.authenticate(r)
		if err != nil {
			w.WriteHeader(http.StatusUnauthorized)
			return
		}
		ctx := context.WithValue(r.Context(), SmurfKey, smurf)
...
func (a *Middleware) authenticate(r *http.Request) (models.Smurf, error) {
	sessionToken, ok := r.Header["Authorization"]
	if !ok {
		return models.Smurf{}, &actions.ErrInvalidSessionToken{}
	}

	return a.usecases.AuthenticateSmurf(sessionToken[0])
}
